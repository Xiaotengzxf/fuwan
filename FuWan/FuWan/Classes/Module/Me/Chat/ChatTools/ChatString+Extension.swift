...
extension String {
    ...
    func isLastStringSystemEmoji() -> Bool {
        ...
        for i in 0..<result.count {
            ...
            if match.range.location + match.range.length == range.length {
                return true
            }
        }
    ...
    func toMsgString() -> NSAttributedString {
        ...
        for i in 0..<result.count {
            let match = result[i] as NSTextCheckingResult
            let range0 = match.range
            let str = NSString(string: self).substring(with: match.range)
            let group = ChatExpression.shared.defaultFace
            ...
            for i in 0..<emojis.count {
                ...
                if emoji.name == str {
                   ...
                   let textAt = NSTextAttachment()
                    ...
                    let  imageDic:[String:AnyObject] = ["Emoji_Image":imageStr,"Emoji_Range":NSValue(range: range0)]
        ...
        for i in 0..<data.count  {
            ...
            let string = (data[i] as![String:AnyObject])["Emoji_Image"] as! NSAttributedString
            attributeStr.replaceCharacters(in: range, with: string)
        }
