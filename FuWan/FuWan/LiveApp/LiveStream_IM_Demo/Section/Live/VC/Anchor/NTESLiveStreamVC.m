...
@interface NTESLiveStreamVC ()<NTESAnchorBottomBarProtocol, NTESAnchorTopBarProtocol, NTESTextInputViewDelegate, NTESMuteViewProtocol, NTESEndViewProtocol, NTESChatViewProtocol, NIMChatManagerDelegate, NIMChatroomManagerDelegate,UIImagePickerControllerDelegate>{
    BOOL _bFinished;
    UIImage * image;
    PHAsset *asset;
}
...
@property (nonatomic, strong) UIButton *uploadPhotoButton; // 上传图片按钮
...
- (void)startLiveAction:(UIButton *)btn
{
    if (image) {
        ...
        NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] multipartFormRequestWithMethod:@"POST" URLString:@"https://www.fuwan369.com/index.php/live/room/public_thumb" parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
            [formData appendPartWithFileData:UIImageJPEGRepresentation(image, 1) name:@"file" fileName:filename mimeType:@"image/jpeg"];
            [formData appendPartWithFormData:[_roomId dataUsingEncoding:NSUTF8StringEncoding] name:@"roomid"];
        } error:nil];
        ...
        uploadTask = [manager
                      ...
                      completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
                          if (error) {
                              ...
                              NSLog(@"上传图片 %@ %@", response, responseObject);
                          }
                      }];
...
{
    if (_bFinished) {
        [self.view makeToast:@"不可重复选择" duration:2.0 position:CSToastPositionCenter];
        return;
    }
...
- (UIButton *)uploadPhotoButton
{
    if (!_uploadPhotoButton) {
        ...
        //_uploadPhotoButton.backgroundColor = [UIColor redColor];
        [_uploadPhotoButton setBackgroundImage:[UIImage imageNamed:@"button_add_phone"] forState:UIControlStateNormal];
        [_uploadPhotoButton addTarget:self action:@selector(selectPhoto:) forControlEvents:UIControlEventTouchUpInside];
    }
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto {
    if (photos.count == 1) {
        ...
        image = photos[0];
        asset = assets[0];
        [self.uploadPhotoButton setImage:photos[0] forState:UIControlStateNormal];
    }
