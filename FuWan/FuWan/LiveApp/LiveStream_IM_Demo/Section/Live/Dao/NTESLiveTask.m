...
- (NSURLRequest *)taskRequest
{
    ...
    NSString *postData = [NSString stringWithFormat:@"sid=%@&deviceId=%@&token=%@",accid, deviceId,token];
...
- (NSURLRequest *)taskRequest
{
    ...
    NSString * token = [[NSUserDefaults standardUserDefaults] stringForKey:@"token"];
    NSString *postData = [NSString stringWithFormat:@"sid=%@&roomId=%@&token=%@",sid, _roomId, token];
    [request setHTTPBody:[postData dataUsingEncoding:NSUTF8StringEncoding]];
    return request;
}
...
- (NSURLRequest *)taskRequest
{
    ...
    NSString * token = [[NSUserDefaults standardUserDefaults] stringForKey:@"token"];
    [params appendString:[NSString stringWithFormat:@"&token=%@", token]];
    if (_roomId) {
